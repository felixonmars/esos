...
struct rpmi_clock_data k1_os0_clk_data[1] = {
	[0] = {
		. parent_id = -1,
	},
};

static int  _k1_os0_clock_init(void *priv)
{
	...
	config->clk_data = k1_os0_clk_data;
...
static struct rpmi_clock_platform_ops k1_os0_clock_pops = {
...
static struct spacemit_rpmi_clk_ops k1_os0_clk_ops = {
	.name = "k1-os0-rpmi-clock",
	.init = _k1_os0_clock_init,
	.clk_ops = &k1_os0_clock_pops,
};

static int k1_os0_clk_init(void)
{

	rt_list_init(&k1_os0_clk_ops.list);

	spacemit_rpmi_clk_register(&k1_os0_clk_ops.list);

	return 0;
}
INIT_DEVICE_EXPORT(k1_os0_clk_init);
