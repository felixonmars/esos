 ...
 *
 * spacemit k3 genphy driver - RT-Thread port
 *
 */
...
struct mii_dev {
	...
	void *priv;
	rt_int32_t (*read)(struct mii_dev *bus, rt_int32_t addr,
			   rt_int32_t devad, rt_int32_t reg);
	rt_int32_t (*write)(struct mii_dev *bus, rt_int32_t addr,
			    rt_int32_t devad, rt_int32_t reg, rt_uint16_t val);
	rt_int32_t (*reset)(struct mii_dev *bus);
...
struct phy_device {
	...
	rt_int32_t addr;
	rt_int32_t pause;
	rt_int32_t asym_pause;
	rt_uint32_t phy_id;
	rt_uint32_t is_c45;
	rt_uint32_t flags;
	rt_bool_t loopback_enabled;

	rt_thread_t link_monitor_thread;
	rt_bool_t monitor_running;
	rt_err_t (*link_change_cb)(void *netdev);
};

struct phy_device *phy_connect(struct mii_dev *bus, int addr,
			       void *dev, phy_interface_t iface);
