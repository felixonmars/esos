...
#else
    base->MCR |= CAN_MCR_WRNEN_MASK | CAN_MCR_MAXMB(FSL_FEATURE_FLEXCAN_HAS_MESSAGE_BUFFER_MAX_NUMBER - 1);
#endif
...
static void FLEXCAN_RamInit(CAN_Type *base)
{
    uint32_t ctrl2;
    uint32_t count;
    int32_t i;
    ...
    addr = (uint32_t *)&base->MB[0];
    for (i = 0; i < CAN_RAM_SIZE / 4; i++)
    {
        *(addr + i) = 0;
    }
...
void FLEXCAN_Init(CAN_Type *base, const flexcan_config_t *config, uint32_t sourceClock_Hz)
{
    ...
    base->FDCTRL |= CAN_FDCTRL_MBDSR0(kFLEXCAN_64BperMB);
    base->FDCTRL |= CAN_FDCTRL_MBDSR1(kFLEXCAN_64BperMB);
...
void FLEXCAN_SetFDRxMbConfig(CAN_Type *base, uint8_t mbIdx, const flexcan_rx_mb_config_t *config, bool enable)
{
    ...
    for (cnt = 0; cnt < FSL_FEATURE_FLEXCAN_HAS_MESSAGE_BUFFER_MAX_NUMBER; cnt++)
    {
        base->MB[cnt].CS = 0;
        base->MB[cnt].ID = 0;
        base->MB[cnt].WORD0 = 0;
...
void FLEXCAN_SetRxFifoConfig(CAN_Type *base, const flexcan_rx_fifo_config_t *config, bool enable)
{
    ...
    if (enable)
    {
        ...
        setup_mb = (setup_mb < FSL_FEATURE_FLEXCAN_HAS_MESSAGE_BUFFER_MAX_NUMBER) ?
                       setup_mb :
                       FSL_FEATURE_FLEXCAN_HAS_MESSAGE_BUFFER_MAX_NUMBER;
...
status_t FLEXCAN_ReadFDRxMb(CAN_Type *base, uint8_t mbIdx, flexcan_fd_frame_t *rxFrame)
{
    ...
    if ((kFLEXCAN_RxMbFull == rx_code) || (kFLEXCAN_RxMbOverrun == rx_code))
    {
        ...
        /* Store Message Payload. */
        for (cnt = 0; cnt < rxFrame->length / 4; cnt++)
        {
...
{
    assert(s_flexcanHandle);

    s_flexcanIsr(param, s_flexcanHandle);
}
