...
/****************************************************************************/
//Review passed. By Yu/2025-09-19
#include "base.h"
	...
	)
{
	...
	for (dev_idx = EC_DEVICE_MAIN;
		...
		rt_snprintf(pair->datagrams[dev_idx].name,
			    EC_DATAGRAM_NAME_SIZE, "domain%u-%lu-%s", domain->index,
	...
	pair->send_buffer = rt_malloc(data_size);
	if (!pair->send_buffer) {
	...
	if (used[EC_DIR_OUTPUT] && used[EC_DIR_INPUT]) { // inputs and outputs
		...
		for (dev_idx = EC_DEVICE_BACKUP;
			dev_idx < ec_master_num_devices(domain->master); dev_idx++)
			ec_datagram_lrw(&pair->datagrams[dev_idx], logical_offset, data_size);

		// If LRW is used, output FMMUs increment the working counter by 2,
		// while input FMMUs increment it by 1.
		pair->expected_working_counter = used[EC_DIR_OUTPUT] * 2 + used[EC_DIR_INPUT];
	} else if (used[EC_DIR_OUTPUT]) { // outputs only
		ec_datagram_lwr_ext(&pair->datagrams[EC_DEVICE_MAIN],
			logical_offset, data_size, data);
		for (dev_idx = EC_DEVICE_BACKUP;
			    dev_idx < ec_master_num_devices(domain->master); dev_idx++)
			ec_datagram_lwr(&pair->datagrams[dev_idx], logical_offset, data_size);
		...
		for (dev_idx = EC_DEVICE_BACKUP;
			    dev_idx < ec_master_num_devices(domain->master); dev_idx++)
			ec_datagram_lrd(&pair->datagrams[dev_idx], logical_offset, data_size);
