	...
	)
{
	...
	if (request->dir == EC_DIR_OUTPUT) {
		...
		fsm->tx_filename_len = rt_strlen((const char *)fsm->tx_filename);
		...
		fsm->rx_filename_len = rt_strlen((const char *)fsm->tx_filename);
	...
	)
{
	...
	rt_memcpy(data + EC_FOE_HEADER_SIZE,
	...
	)
{
	...
	rt_memcpy(data + EC_FOE_HEADER_SIZE, fsm->tx_filename, current_size);
	...
	)
{
	...
	rt_memcpy(data + EC_FOE_HEADER_SIZE, fsm->rx_filename, current_size);
	...
	)
{
	...
	if (opCode == EC_FOE_OPCODE_ERR) {
		...
		if (rec_size > 6) {
			uint8_t text[256];
			rt_strncpy((char *)(text), (const char *)(data + 6), min(rec_size - 6, sizeof(text)));
	...
	if (fsm->rx_buffer_size >= fsm->rx_buffer_offset + rec_size) {
		rt_memcpy(fsm->rx_buffer + fsm->rx_buffer_offset,
	...
	if (fsm->rx_last_packet ||
		...
		rt_kprintf("  rx_buffer_size = %d\n", fsm->rx_buffer_size);
		rt_kprintf("rx_buffer_offset = %d\n", fsm->rx_buffer_offset);
		rt_kprintf("        rec_size = %zd\n", rec_size);
		rt_kprintf(" rx_mailbox_size = %d\n", slave->configured_rx_mailbox_size);
		rt_kprintf("  rx_last_packet = %d\n", fsm->rx_last_packet);
