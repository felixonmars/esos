	...
	)
{
	...
	slave->sii.name = NULL;
	rt_memset(&slave->sii.coe_details, 0x00, sizeof(ec_sii_coe_details_t));
	rt_memset(&slave->sii.general_flags, 0x00, sizeof(ec_sii_general_flags_t));
...
void ec_slave_clear(ec_slave_t *slave /**< EtherCAT slave */)
{
	...
	if (slave->sii.strings) {
		for (i = 0; i < slave->sii.string_count; i++)
			rt_free(slave->sii.strings[i]);
		rt_free(slave->sii.strings);
	}
	...
	)
{
	...
	if (slave->sii.string_count) {
		slave->sii.strings = rt_malloc(sizeof(char *) * slave->sii.string_count);
		if (!slave->sii.strings) {
		...
		for (i = 0; i < slave->sii.string_count; i++) {
			...
			slave->sii.strings[i] = rt_malloc(sizeof(char) * size + 1);
			if (!slave->sii.strings[i]) {
				EC_SLAVE_ERR(slave, "Failed to allocate string memory.\n");
				err = -ENOMEM;
				goto out_free;
			}
			rt_memcpy(slave->sii.strings[i], data + offset + 1, size);
	...
	)
{
	...
	while (data_size >= 8) {
		...
		for (i = 0; i < entry_count; i++) {
			entry = rt_malloc(sizeof(ec_pdo_entry_t));
			if (!entry) {
		...
		if (pdo->sync_index >= 0) {
			ec_sync_t *sync;
			sync = ec_slave_get_sync(slave, pdo->sync_index);
			if (!sync) {
...
void ec_slave_sdo_dict_info(const ec_slave_t *slave, /**< EtherCAT slave */
	...
	unsigned int *entry_count /**< total number of entries */
	)
{
	...
	)
{
	...
	for (i = 0; i < slave->sii.sync_count; i++) {
		...
		pdo = ec_pdo_list_find_pdo_const(&sync->pdos, index);
		if (!pdo)
			continue;
