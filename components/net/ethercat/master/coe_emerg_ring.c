	...
	)
{
	...
	ring->msgs = rt_malloc(sizeof(ec_coe_emerg_msg_t) * (size + 1));
	if (!ring->msgs)
		return -ENOMEM;
	...
	)
{
	...
	rt_memcpy(ring->msgs[ring->write_index].data, msg,
	...
	)
{
	...
	rt_memcpy(msg, ring->msgs[ring->read_index].data, EC_COE_EMERGENCY_MSG_SIZE);
