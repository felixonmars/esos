 ...
 *
 *  The IgH EtherCAT Master is rt_free software; you can redistribute it and/or
 *  modify it under the terms of the GNU General Public License version 2, as
 *  published by the rt_free Software Foundation.
 *
 ...
 *  with the IgH EtherCAT Master; if not, write to the rt_free Software
...
#define RT_EC_THREAD_STACK_SIZE 65535

#define RT_MASTER_THREAD_PRIORITY 8

#define RT_EC_THREAD_TICK 10
...
int ec_master_thread_start(ec_master_t *, void (*)(void *), const char *);
...
//	sema_init(master->master_sem, 1);
	master->master_sem = ec_sema_init(1);
	...
	master->device_sem = ec_sema_init(1);
	...
	master->scan_sem = ec_sema_init(1);

	init_waitqueue_head(&master->scan_queue);

	master->config_busy = 0;

	master->config_sem = ec_sema_init(1);
	...
	master->ext_queue_sem = ec_sema_init(1);
	...
	for (i = 0; i < EC_EXT_RING_SIZE; i++) {
		...
		rt_snprintf(datagram->name, EC_DATAGRAM_NAME_SIZE, "ext-%u", i);
	}
	...
	master->io_mutex = ec_mutex_init(master->io_mutex);
	master->send_cb = NULL;
	...
	rt_snprintf(master->fsm_datagram.name, EC_DATAGRAM_NAME_SIZE, "master-fsm");
	...
	rt_snprintf(master->ref_sync_datagram.name, EC_DATAGRAM_NAME_SIZE,
		"refsync");
	...
	rt_snprintf(master->sync_datagram.name, EC_DATAGRAM_NAME_SIZE, "sync");
	...
	rt_snprintf(master->sync_mon_datagram.name, EC_DATAGRAM_NAME_SIZE,
		"syncmon");
	...
	)
{
	...
	master->thread = rt_thread_create(name, thread_func, master,
		RT_EC_THREAD_STACK_SIZE, RT_MASTER_THREAD_PRIORITY, RT_EC_THREAD_TICK);
	if (master->thread == RT_NULL) {
		EC_MASTER_ERR(master, "Failed to create master thread!\n");
		return -1;
	}
	...
	rt_thread_startup(master->thread);
	return 0;
}
	...
	)
{
	...
	result = rt_thread_delete(master->thread);  // Equivalent to kthread_stop
	if (result != RT_EOK) {
		EC_MASTER_WARN(master, "%s(): Failed to stop master thread.\n", __func__);
	}
	...
	rt_thread_mdelay(sleep_jiffies);
}
...
#if DEBUG_INJECT
	EC_MASTER_DBG(master, 1, "Injecting datagrams, queue_size=%u\n",
			queue_size);
#endif
	...
	while (master->ext_ring_idx_rt != master->ext_ring_idx_fsm) {
		...
		if (new_queue_size <= master->max_queue_size) {
			...
			if (cycles_now - datagram->cycles_sent
					> ext_injection_timeout_cycles)
#else
			if (rt_tick_get() - datagram->jiffies_sent
					> ext_injection_timeout_jiffies)
#endif
			{
...
#else
				time_us = (unsigned int)
					((rt_tick_get() - datagram->jiffies_sent) * 1000000 / HZ);
#endif
				EC_MASTER_ERR(master, "Timeout %u us: Injecting"
						" external datagram %s size=%u,"
...
/** Searches for a rt_free datagram in the external datagram ring.
 *
 * \return Next rt_free datagram, or NULL.
 */
	...
	)
{
	...
	do {
		...
		list_for_each_entry(datagram, &master->datagram_queue, queue) {
			...
			rt_memcpy(cur_data + 2, datagram->address, EC_ADDR_LEN);

			/* Split the write operation to avoid a potential store address misalignment error */
			EC_WRITE_U8(cur_data + 6, datagram->data_size & 0xFF);
			EC_WRITE_U8(cur_data + 7, (datagram->data_size >> 8) & 0x7);
			/* to avoid potential store address misalignment */
			rt_memset(cur_data + 8, 0, EC_FRAME_STATUS_SIZE);
			follows_word = cur_data + 6;
			cur_data += EC_DATAGRAM_HEADER_SIZE;

			// EtherCAT datagram data
			rt_memcpy(cur_data, datagram->data, datagram->data_size);
			cur_data += datagram->data_size;

			// EtherCAT datagram footer
			/* to avoid potential store address misalignment */
			rt_memset(cur_data, 0, EC_FRAME_WC_SIZE); // reset working counter
		...
		EC_MASTER_DBG(master, 2, "frame size: %u\n", cur_data - frame_data);
	...
	)
{
	...
	if (device->master->active && 0) {
		rt_kprintf("in ec_master_receive_datagrams\n");
	...
	while (cmd_follows) {
		...
		if (datagram->type != EC_DATAGRAM_APWR &&
			...
			rt_memcpy(datagram->data, cur_data, data_size);
		}
		cur_data += data_size;
...
{
	if (unlikely(rt_tick_get() - master->stats.output_jiffies >= HZ)) {
		master->stats.output_jiffies = rt_tick_get();
	...
	)
{
	...
	// frame statistics
	if (likely(rt_tick_get() - s->jiffies < HZ)) {
		return;
	}
		...
		)
{
	...
	list_for_each_entry_safe(fsm, next, &master->fsm_exec_list, list) {
		...
		if (!datagram) {
			// no rt_free datagrams at the moment
			EC_MASTER_WARN(master, "No rt_free datagram during"
...
static void ec_master_operation_thread(void *priv_data)
{
	...
	EC_MASTER_DBG(master, 1, "Operation thread running"
		" with fsm interval = %u us, max data size=%u\n",
...
/** Same as ecrt_master_create_domain(), but with err_to_ptr() return value.
 *
 * \return New domain, or err_to_ptr() return value.
 */
	...
	)
{
	...
	domain = (ec_domain_t *) rt_malloc(sizeof(ec_domain_t));
	if (!domain) {
...
/** Same as ecrt_master_slave_config(), but with err_to_ptr() return value.
 */
		...
		uint32_t product_code)
{
	...
	if (found) { // config with same alias/position already existing
		...
		sc = (ec_slave_config_t *) rt_malloc_align(sizeof(ec_slave_config_t), 8);
		if (!sc) {
		...
		ec_slave_config_t *sc)
{
	if (sc) {
		...
		if (slave && (!slave->base_dc_supported || !slave->has_dc_system_time)) {
		...
		ec_slave_info_t *slave_info)
{
	...
	if (slave->sii.name) {
		rt_strncpy(slave_info->name, slave->sii.name, EC_MAX_STRING_LENGTH);
	} else {
		slave_info->name[0] = 0;
	}
		...
		size_t data_size, uint32_t *abort_code)
{
	...
	EC_MASTER_DBG(master, 1, "%s(master = 0x%p,"
		...
		" data = 0x%p, data_size = %u, abort_code = 0x%p)\n",
	...
	rt_memcpy(request.data, data, data_size);
	...
	slave = ec_master_find_slave(master, 0, slave_position);
	if (!slave) {
		up(master->master_sem);
		...
		size_t target_size, size_t *result_size, uint32_t *abort_code)
{
	...
	EC_MASTER_DBG(master, 1, "%s(master = 0x%p,"
		" slave_position = %u, index = 0x%04X, subindex = 0x%02X,"
		" target = 0x%p, target_size = %u, result_size = 0x%p,"
		" abort_code = 0x%p)\n",
		__func__, master, slave_position, index, subindex,
	...
	if (request.state != EC_INT_REQUEST_SUCCESS) {
		*result_size = 0;
		...
		if (request.data_size > target_size) {
			...
			rt_memcpy(target, request.data, request.data_size);
