	...
	)
{
	...
	if (!list_empty(&master->emerg_reg_requests)) {
		...
		rt_memcpy(fsm->datagram->data, request->data, request->transfer_size);
	...
	)
{
	...
	if (fsm->rescan_required) {
		...
		if (!master->allow_scan) {
			...
			master->slaves = (ec_slave_t *) rt_malloc(size);
			if (!master->slaves) {
	...
	)
{
	...
	for (slave = master->slaves;
		...
		if (!(slave->sii.mailbox_protocols & EC_MBOX_COE)
				...
				|| rt_tick_get() - slave->jiffies_preop < EC_WAIT_SDO_DICT * HZ
				)
			continue;
	...
	)
{
	...
	EC_MASTER_INFO(master, "Bus scanning completed in %lu ms.\n",
		(rt_tick_get() - fsm->scan_jiffies) * 1000 / HZ);
		...
		)
{
	...
	// correct read system time by elapsed time since read operation
	correction = (uint64_t) (jiffies_since_read * 1000 / HZ) * 1000000;
	...
	)
{
	...
	jiffies_since_read = rt_tick_get() - datagram->jiffies_sent;
