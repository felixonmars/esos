	...
	)
{
	...
	if (pdo->name && name && !rt_strcmp(pdo->name, name))
		return 0;
	...
	len = rt_strlen(name);
	if (name && len) {
		pdo->name = (char *) rt_malloc(len + 1);
		if (!pdo->name) {
			EC_ERR("Failed to allocate PDO name.\n");
			return -ENOMEM;
		}
		rt_memcpy(pdo->name, name, len + 1);
	} else {
		pdo->name = NULL;
	}
	...
	)
{
	...
	list_for_each_entry(other_entry, &other->entries, list) {
		entry = (ec_pdo_entry_t *)(rt_malloc(sizeof(ec_pdo_entry_t)));
		if (!entry) {
...
{
	const list_head *head1, *head2, *item1, *item2;
