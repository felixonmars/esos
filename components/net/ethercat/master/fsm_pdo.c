...
{
	rt_kprintf("Currently assigned PDOs: ");
	ec_pdo_list_print(&fsm->sync->pdos);
	rt_kprintf(". PDOs to assign: ");
	...
	)
{
	...
	for (; fsm->sync_index < EC_MAX_SYNC_MANAGERS; fsm->sync_index++) {
		fsm->sync = ec_slave_get_sync(slave, fsm->sync_index);
		if (!fsm->sync)
			continue;
	...
	)
{
	...
	fsm->pdo = (ec_pdo_t *)rt_malloc(sizeof(ec_pdo_t));
	if (!fsm->pdo) {
...
ec_pdo_t *ec_fsm_pdo_conf_action_next_pdo(
	const ec_fsm_pdo_t *fsm, /**< PDO configuration state machine. */
	const list_head *list /**< current PDO list item */
	)
{
	list = list->next;
	...
	)
{
	...
	for (; fsm->sync_index < EC_MAX_SYNC_MANAGERS; fsm->sync_index++) {
		...
		fsm->sync = ec_slave_get_sync(fsm->slave, fsm->sync_index);
		if (!fsm->sync) {
		...
		fsm->pdo = ec_fsm_pdo_conf_action_next_pdo(fsm, &fsm->pdos.list);
		if (!fsm->pdo) {
			// no pdos configured
	...
	)
{
	...
	assigned_pdo = ec_slave_find_pdo(fsm->slave, fsm->pdo->index);
	if (assigned_pdo) {
	...
	)
{
	...
	if ((fsm->slave->sii.mailbox_protocols & EC_MBOX_COE)
		...
		EC_SLAVE_WARN(fsm->slave, "");
		rt_kprintf("Currently mapped PDO entries: ");
		ec_pdo_print_entries(&fsm->slave_pdo);
		rt_kprintf(". Entries to map: ");
	...
	)
{
	...
	fsm->pdo = ec_fsm_pdo_conf_action_next_pdo(fsm, &fsm->pdo->list);
	if (!fsm->pdo) {
