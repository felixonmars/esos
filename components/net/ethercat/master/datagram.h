...
/**
 * Defines the alignment size for the datagram's data buffer.
 * Set to 8 bytes to ensure proper alignment for 64-bit data.
 */
#define EC_DATAGRAM_ALIGN_SIZE 8
 ...
 */
typedef struct {
	list_head queue; /**< Master datagram queue item,
	    protected by user-supplied mutex. */
	list_head ext_queue; /**< External datagram queue item, protected by ext_queue_sem. */
	list_head sent; /**< Master list item for sent datagrams. */
	...
	uint64_t cycles_sent; /**< Time, when the datagram was sent. */
#endif
	unsigned long jiffies_sent; /**< Jiffies, when the datagram was sent. */
#ifdef EC_HAVE_CYCLES
	uint64_t cycles_received; /**< Time, when the datagram was received. */
#endif
