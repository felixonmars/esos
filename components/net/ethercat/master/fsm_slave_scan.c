...
alloc_sii:
	...
	slave->sii_words = (uint16_t *) rt_malloc(slave->sii_nwords * 2);
	if (!slave->sii_words) {
	...
	/**< slave state machine */)
{
	...
	if (fsm->sii_offset + 2 <= slave->sii_nwords) { // 2 words fit
		rt_memcpy(slave->sii_words + fsm->sii_offset, fsm->fsm_sii.value, 4);
	} else { // copy the last word
		rt_memcpy(slave->sii_words + fsm->sii_offset, fsm->fsm_sii.value, 2);
	}
