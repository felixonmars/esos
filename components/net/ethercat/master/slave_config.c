		...
		)
{
	...
	slave = ec_master_find_slave(sc->master, sc->alias, sc->position);
	if (!slave) {
...
void ec_slave_config_load_default_sync_config(ec_slave_config_t *sc)
{
	...
	for (sync_index = 0; sync_index < EC_MAX_SYNC_MANAGERS; sync_index++) {
		...
		sync = ec_slave_get_sync(sc->slave, sync_index);
		if (sync) {
		...
		uint8_t entry_bit_length)
{
	...
	for (sync_index = 0; sync_index < EC_MAX_SYNC_MANAGERS; sync_index++) {
		pdo = ec_pdo_list_find_pdo(&sc->sync_configs[sync_index].pdos, pdo_index);
		if (pdo)
			break;
	}
		...
		uint8_t subindex, const uint8_t *data, size_t size)
{
	...
	EC_CONFIG_DBG(sc, 1, "%s(sc = 0x%p, index = 0x%04X, "
			"subindex = 0x%02X, data = 0x%p, size = %u)\n",
	...
	req = (ec_sdo_request_t *) rt_malloc(sizeof(ec_sdo_request_t));
	if (!req) {
...
/** Same as ecrt_slave_config_create_sdo_request(), but with err_to_ptr() return
 * value.
 */
		...
		ec_slave_config_t *sc, uint16_t index, uint8_t subindex, size_t size)
{
	ec_sdo_request_t *s = ecrt_slave_config_create_sdo_request_err(sc, index,
			subindex, size);
	return ptr_is_err(s) ? NULL : s;
}

/****************************************************************************/

/** Same as ecrt_slave_config_create_soe_request(), but with err_to_ptr() return
 * value.
 */
		...
		ec_slave_config_t *sc, uint8_t drive_no, uint16_t idn, size_t size)
{
	...
	EC_CONFIG_DBG(sc, 1, "%s(sc = 0x%p, "
			"drive_no = 0x%02X, idn = 0x%04X, size = %u)\n",
			__func__, sc, drive_no, idn, size);

	req = (ec_soe_request_t *) rt_malloc(sizeof(ec_soe_request_t));
	if (!req) {
		...
		ec_slave_config_t *sc, uint8_t drive_no, uint16_t idn, size_t size)
{
	ec_soe_request_t *req = ecrt_slave_config_create_soe_request_err(sc,
			drive_no, idn, size);
	return ptr_is_err(req) ? NULL : req;
}

/****************************************************************************/

/** Same as ecrt_slave_config_create_reg_request(), but with err_to_ptr() return
 * value.
 */
		...
		ec_slave_config_t *sc, size_t size)
{
	...
	EC_CONFIG_DBG(sc, 1, "%s(sc = 0x%p, size = %u)\n",
			__func__, sc, size);

	reg = (ec_reg_request_t *) rt_malloc(sizeof(ec_reg_request_t));
	if (!reg) {
		...
		ec_slave_config_t *sc, size_t size)
{
	...
	return ptr_is_err(reg) ? NULL : reg;
}

/****************************************************************************/

/** Same as ecrt_slave_config_create_voe_handler(), but with err_to_ptr() return
 * value.
 */
		...
		ec_slave_config_t *sc, size_t size)
{
	...
	voe = (ec_voe_handler_t *) rt_malloc(sizeof(ec_voe_handler_t));
	if (!voe) {
		...
		ec_slave_config_t *sc, size_t size)
{
	ec_voe_handler_t *voe = ecrt_slave_config_create_voe_handler_err(sc,
			size);
	return ptr_is_err(voe) ? NULL : voe;
}
		...
		int32_t value)
{
	...
	if (flag) {
		...
		flag = (ec_flag_t *) rt_malloc(sizeof(ec_flag_t));
		if (!flag) {
		...
		ec_al_state_t from, ec_al_state_t to, unsigned int timeout_ms)
{
	...
	timeout = (ec_al_timeout_t *)rt_malloc(sizeof(ec_al_timeout_t));
	if (!timeout) {
		EC_CONFIG_ERR(sc, "Failed to allocate memory for"
