	...
	)
{
	...
	if (rec_size < index_list_offset || rec_size % 2) {
		EC_SLAVE_ERR(slave, "Invalid data size %u!\n", rec_size);
		ec_print_data(data, rec_size);
		fsm->state = ec_fsm_coe_error;
		return;
	}
	...
	for (i = 0; i < sdo_count; i++) {
		...
		sdo = (ec_sdo_t *) rt_malloc(sizeof(ec_sdo_t));
		if (!sdo) {
	...
	)
{
	...
	if (name_size) {
		sdo->name = rt_malloc(name_size + 1);
		if (!sdo->name) {
		...
		rt_memcpy(sdo->name, data + 12, name_size);
	...
	)
{
	...
	if (EC_READ_U16(data) >> 12 == 0x8 && // SDO information
		...
		entry = (ec_sdo_entry_t *) rt_malloc(sizeof(ec_sdo_entry_t));
		if (!entry) {
		...
		if (data_size) {
			char *desc = rt_malloc(data_size + 1);
			if (!desc) {
				EC_SLAVE_ERR(slave, "Failed to allocate SDO entry name!\n");
				fsm->state = ec_fsm_coe_error;
				return;
			}
			rt_memcpy(desc, data + 16, data_size);
			desc[data_size] = 0;
	...
	)
{
	...
	if (request->data_size > 0 && request->data_size <= 4) {
		...
		if (ptr_is_err(data)) {
			request->error_code = ptr_to_err(data);
			return ptr_to_err(data);
		}
...
//		EC_WRITE_U16(data + 3, request->index);
		...
		EC_WRITE_U8(data + 3, request->index & 0xFF);
		EC_WRITE_U8(data + 4, (request->index >> 8) & 0xFF);

		EC_WRITE_U8 (data + 5,
				request->complete_access ? 0x00 : request->subindex);
		rt_memcpy(data + 6, request->data, request->data_size);
		rt_memset(data + 6 + request->data_size, 0x00, 4 - request->data_size);
...
//		EC_WRITE_U16(data + 3, request->index);
		...
		if (data_size > EC_COE_DOWN_REQ_HEADER_SIZE) {
			...
			rt_memcpy(data + EC_COE_DOWN_REQ_HEADER_SIZE,
	...
	)
{
	...
	if (fsm->slave->master->debug_level) {
		...
		if (request->complete_access) {
			...
			rt_sprintf(subidxstr, ":%02X", request->subindex);
		}
	...
	if (slave->configured_rx_mailbox_size <
		...
		request->error_code = ENOBUFS;
		fsm->state = ec_fsm_coe_error;
		return;
	}


	fsm->request->jiffies_sent = rt_tick_get();
	fsm->retries = EC_FSM_RETRIES;
	...
	)
{
	...
	diff_ms = (rt_tick_get() - fsm->request->jiffies_sent) * 1000 / HZ;
	...
	)
{
	...
	rt_memcpy(data + EC_COE_DOWN_SEG_REQ_HEADER_SIZE,
			request->data + fsm->offset, fsm->segment_size);
	if (fsm->segment_size < EC_COE_DOWN_SEG_MIN_DATA_SIZE) {
		rt_memset(data + EC_COE_DOWN_SEG_REQ_HEADER_SIZE + fsm->segment_size,
	...
	)
{
	...
	if (rec_size < 6) {
		request->error_code = EIO;
		fsm->state = ec_fsm_coe_error;
		EC_SLAVE_ERR(slave, "Received data are too small (%u bytes):\n",
			rec_size);
		ec_print_data(data, rec_size);
		return;
	}
	...
	if (EC_READ_U16(data) >> 12 == 0x2 && // SDO request
		...
		EC_SLAVE_ERR(slave, "SDO download 0x%04X%s (%u bytes) aborted.\n",
	...
	)
{
	uint8_t *data =
		ec_slave_mbox_prepare_send(fsm->slave, datagram, EC_MBOX_TYPE_COE,
				10);
	if (ptr_is_err(data)) {
		fsm->request->error_code = ptr_to_err(data);
		fsm->state = ec_fsm_coe_error;
		return;
	}
	...
	)
{
	...
	if (expedited) {
		...
		if (data_size < fsm->complete_size) {
			EC_SLAVE_DBG(slave, 1, "SDO data incomplete (%u / %u)."
	...
	)
{
	...
	rt_memcpy(request->data + request->data_size, data + 3, data_size);
