...
/**
   ...
   \return Pointer to mailbox datagram data, or err_to_ptr() code.
*/
				    ...
				    )
{
	...
	if (unlikely(!slave->sii.mailbox_protocols)) {
		EC_SLAVE_ERR(slave, "Slave does not support mailbox"
			" communication!\n");
		return err_to_ptr(-EPROTONOSUPPORT);
	}
 ...
 *
 * \return Pointer to the received data, or err_to_ptr() code.
 */
	...
	if (*type == 0x00) {
		...
		for (mbox_msg = mbox_error_messages; mbox_msg->code; mbox_msg++) {
			if (mbox_msg->code != code)
				continue;
			rt_kprintf("Code 0x%04X: \"%s\".\n",
					mbox_msg->code, mbox_msg->message);
			break;
		}

		if (!mbox_msg->code) {
			rt_kprintf("Unknown error reply code 0x%04X.\n", code);
		}
