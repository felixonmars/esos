...
void ec_datagram_init(ec_datagram_t *datagram /**< EtherCAT datagram. */)
{
	...
	rt_memset(datagram->address, 0x00, EC_ADDR_LEN);
	datagram->data = NULL;
	...
	rt_memset(datagram->name, 0x00, EC_DATAGRAM_NAME_SIZE);
}
	...
	)
{
	...
	datagram->data = rt_malloc_align(size, EC_DATAGRAM_ALIGN_SIZE);
	if (!datagram->data) {
...
{
	rt_memset(datagram->data, 0x00, datagram->data_size);
}
	...
	)
{
	if (datagram->working_counter == 0)
		...
		rt_kprintf("%u slaves responded!", datagram->working_counter);
	else
		rt_kprintf("Success.");
...
{
	if (rt_tick_get() - datagram->stats_output_jiffies > RT_TICK_PER_SECOND) {
		datagram->stats_output_jiffies = rt_tick_get();
