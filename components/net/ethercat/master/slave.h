...
#define EC_SLAVE_INFO(slave, fmt, args...) \
	rt_kprintf("EtherCAT %u-%u: " fmt, slave->master->index, \
...
#define EC_SLAVE_ERR(slave, fmt, args...) \
	rt_kprintf("EtherCAT ERROR %u-%u: " fmt, slave->master->index, \
		   slave->ring_position, ##args)
...
#define EC_SLAVE_WARN(slave, fmt, args...) \
	rt_kprintf("EtherCAT WARNING %u-%u: " fmt, \
...
#define EC_SLAVE_DBG(slave, level, fmt, args...) \
	do { \
		if (slave->master->debug_level >= level) { \
			rt_kprintf("EtherCAT DEBUG %u-%u: " fmt, \
				   slave->master->index, slave->ring_position, ##args); \
		} \
	} while (0)
...
typedef struct {
	...
	// [RT]XPDO
	list_head pdos; /**< SII [RT]XPDO categories. */
} ec_sii_t;
...
struct ec_slave {
	...
	list_head sdo_dictionary; /**< SDO dictionary list */
	uint8_t sdo_dictionary_fetched; /**< Dictionary has been fetched. */
	unsigned long jiffies_preop; /**< Time, the slave went to PREOP. */

	list_head sdo_requests; /**< SDO access requests. */
	list_head reg_requests; /**< Register access requests. */
	list_head foe_requests; /**< FoE requests. */
	list_head soe_requests; /**< SoE requests. */
