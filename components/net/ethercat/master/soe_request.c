	...
	)
{
	...
	req->data = (uint8_t *) rt_malloc(size);
	if (!req->data) {
	...
	)
{
	...
	rt_memcpy(req->data, source, size);
	req->data_size = size;
	return 0;
}
	...
	)
{
	if (req->data_size + size > req->mem_size) {
		...
		uint8_t *new_data = (uint8_t *) rt_malloc(new_size);
		if (!new_data) {
			EC_ERR("Failed to allocate %zu bytes of SoE memory.\n",
					new_size);
			return -ENOMEM;
		}
		rt_memcpy(new_data, req->data, req->data_size);
		rt_free(req->data);
		req->data = new_data;
		req->mem_size = new_size;
	}

	rt_memcpy(req->data + req->data_size, source, size);
	req->data_size += size;
	return 0;
}
