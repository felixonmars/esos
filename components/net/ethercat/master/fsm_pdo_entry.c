	...
	)
{
	...
	if (fsm->request.data_size != sizeof(uint32_t)) {
		...
		pdo_entry = (ec_pdo_entry_t *)rt_malloc(sizeof(ec_pdo_entry_t));
		if (!pdo_entry) {
...
ec_pdo_entry_t *ec_fsm_pdo_entry_conf_next_entry(
	const ec_fsm_pdo_entry_t *fsm, /**< PDO mapping state machine. */
	const list_head *list /**< current entry list item */
	)
{
	list = list->next;
	...
	)
{
	...
	// find first entry
	fsm->entry = ec_fsm_pdo_entry_conf_next_entry(fsm, &fsm->source_pdo->entries);
	if (!fsm->entry) {
	...
	)
{
	...
	// find next entry
	fsm->entry = ec_fsm_pdo_entry_conf_next_entry(fsm, &fsm->entry->list);
	if (!fsm->entry) {
