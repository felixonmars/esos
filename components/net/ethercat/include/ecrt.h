...
#include <stdint.h> // for fixed-width integer types like uint8_t, int32_t, etc.
#include <stddef.h> // for size_t and NULL
#include <errno.h>  // For error number macros

/* RT-Thread version keeps kernel-space interfaces only. */
#ifndef __KERNEL__
#define __KERNEL__
#endif
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_U16(DATA) \
	((uint16_t) le16_to_cpup((void *) (DATA)))

#define EC_READ_U16(DATA) ec_safe_read_u16(DATA)

static inline uint16_t ec_safe_read_u16(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (likely(data_ptr % 2 == 0)) {
		return le16_to_cpup((void *)addr);
	}

	return (uint16_t)(*(uint8_t *)(addr)) |
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_S16(DATA) \
	((int16_t) le16_to_cpup((void *) (DATA)))

#define EC_READ_S16(DATA) ec_safe_read_s16(DATA)

static inline int16_t ec_safe_read_s16(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (likely(data_ptr % 2 == 0)) {
		return (int16_t)le16_to_cpup((void *)addr);
	}

	uint16_t tmp_data = (uint16_t)(*(uint8_t *)(addr)) |
			    ((uint16_t)(*(uint8_t *)((addr) + 1)) << 8);
	return (int16_t)tmp_data;
}
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_U32(DATA) \
	((uint32_t) le32_to_cpup((void *) (DATA)))

#define EC_READ_U32(DATA) ec_safe_read_u32(DATA)

static inline uint32_t ec_safe_read_u32(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (likely(data_ptr % 4 == 0)) {
		return le32_to_cpup((void *)addr);
	}
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_S32(DATA) \
	((int32_t) le32_to_cpup((void *) (DATA)))

#define EC_READ_S32(DATA) ec_safe_read_s32(DATA)

static inline int32_t ec_safe_read_s32(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (likely(data_ptr % 4 == 0)) {
		return (int32_t) le32_to_cpup((void *)addr);
	}
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_U64(DATA) \
	((uint64_t) le64_to_cpup((void *) (DATA)))

#define EC_READ_U64(DATA) ec_safe_read_u64(DATA)

static inline uint64_t ec_safe_read_u64(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (data_ptr % 8 == 0) {
		return le64_to_cpup((void *)addr);
	}
 ...
 * \return EtherCAT data value
 */
#define EC_FAST_READ_S64(DATA) \
	((int64_t) le64_to_cpup((void *) (DATA)))

#define EC_READ_S64(DATA) ec_safe_read_s64(DATA)

static inline int64_t ec_safe_read_s64(const void *addr)
{
	uintptr_t data_ptr = (uintptr_t)addr;

	if (data_ptr % 8 == 0) {
		return (int64_t) le64_to_cpup((void *)addr);
	}
 ...
 * \param VAL new value
 */
#define EC_FAST_WRITE_U16(DATA, VAL) \
	do { \
		*((uint16_t *) (DATA)) = cpu_to_le16((uint16_t) (VAL)); \
	} while (0)

#define EC_WRITE_U16(DATA, VAL) ec_safe_write_u16(DATA, VAL)

static inline void ec_safe_write_u16(void *data, uint16_t val)
{
	uintptr_t data_ptr = (uintptr_t)data;

	if (likely(data_ptr % 2 == 0)) {
		*((uint16_t *)data) = cpu_to_le16(val);
	} else {
		uint8_t *byte_data = (uint8_t *)data;

		byte_data[0] = (uint8_t)(val);
		byte_data[1] = (uint8_t)(val >> 8);
	}
}
 ...
 * \param VAL new value
 */
#define EC_FAST_WRITE_S16(DATA, VAL) EC_FAST_WRITE_U16(DATA, VAL)
 ...
 * \param VAL new value
 */
#define EC_FAST_WRITE_U32(DATA, VAL) \
	do { \
		*((uint32_t *) (DATA)) = cpu_to_le32((uint32_t) (VAL)); \
	} while (0)

#define EC_WRITE_U32(DATA, VAL) ec_safe_write_u32(DATA, VAL)

static inline void ec_safe_write_u32(void *data, uint32_t val)
{
	uintptr_t data_ptr = (uintptr_t)data;

	if (likely(data_ptr % 4 == 0)) {
		*((uint32_t *)data) = cpu_to_le32(val);
	} else {
		uint8_t *byte_data = (uint8_t *)data;
		for (int i = 0; i < 4; i++)
			byte_data[i] = (uint8_t)(val >> (i * 8));
	}
}
 ...
 * \param VAL new value
 */
#define EC_FAST_WRITE_S32(DATA, VAL) EC_FAST_WRITE_U32(DATA, VAL)
...
#define EC_FAST_WRITE_U64(DATA, VAL) \
	do { \
		*((uint64_t *) (DATA)) = cpu_to_le64((uint64_t) (VAL)); \
	} while (0)

#define EC_WRITE_U64(DATA, VAL) ec_safe_write_u64(DATA, VAL)

static inline void ec_safe_write_u64(void *data, uint64_t val)
{
	uintptr_t data_ptr = (uintptr_t)data;

	if (data_ptr % 8 == 0) {
		*((uint64_t *)data) = cpu_to_le64(val);
	} else {
		uint8_t *byte_data = (uint8_t *)data;

		byte_data[0] = (uint8_t)(val);
		byte_data[1] = (uint8_t)(val >> 8);
		byte_data[2] = (uint8_t)(val >> 16);
		byte_data[3] = (uint8_t)(val >> 24);
		byte_data[4] = (uint8_t)(val >> 32);
		byte_data[5] = (uint8_t)(val >> 40);
		byte_data[6] = (uint8_t)(val >> 48);
		byte_data[7] = (uint8_t)(val >> 56);
	}
}
 ...
 * \param VAL new value
 */
#define EC_FAST_WRITE_S64(DATA, VAL) EC_FAST_WRITE_U64(DATA, VAL)
