...
#define IPC_DEV_NAME         "ipc_dev"
#define IPC_BUS_NAME         "generic"
 ...
 */
static metal_phys_addr_t ipc_phys_addr = IPC_BASE_ADDR;

struct metal_device ipc_device = {
	.name = "ipc_dev",
	.bus = NULL,
	.num_regions = 1,
	.regions = {
		{
			.virt = (void *)IPC_BASE_ADDR,
			.physmap = &ipc_phys_addr,
			.size = 0x100,
			.page_shift = -1UL,
			.page_mask = -1UL,
...
#ifndef RPMSG_NO_IPI
	.irq_num = 1,
	.irq_info = (void *)IPC_IRQ_VECT_ID,
#endif /* !RPMSG_NO_IPI */
};

static struct remoteproc_priv rproc_priv = {
	.poll_dev_name = IPC_DEV_NAME,
	.poll_dev_bus_name = IPC_BUS_NAME,
#ifndef RPMSG_NO_IPI
	.ipi_chn_mask = IPC_CHN_BITMASK,
#endif /* !RPMSG_NO_IPI */
};
...
extern struct remoteproc_ops spacemit_k1x_proc_ops;
...
#ifndef RPMSG_NO_IPI
	/* Register IPI device */
	ret = metal_register_generic_device(&ipc_device);
	if (ret)
		return NULL;
#endif /* !RPMSG_NO_IPI */

	/* Initialize remoteproc instance */
	if (!remoteproc_init(&rproc_inst, &spacemit_k1x_proc_ops, &rproc_priv))
		return NULL;
...
int platform_init(int argc, char *argv[], void **platform)
{
	...
	if (!platform) {
		metal_err("Failed to initialize platform,"
...
int platform_poll(void *priv)
{
	...
	while(1) {
		...
		metal_irq_restore_enable(flags);
#else
		rt_sem_take(prproc->sem, RT_WAITING_FOREVER);
