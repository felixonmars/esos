...
rt_err_t rt_regulator_register(struct rt_regulator_node *reg_np)
{
    ...
    rt_mutex_init(&reg_np->mutex, "reg_mut", RT_IPC_FLAG_FIFO);
    reg_np->enabled_count = 0;
    reg_np->open_count = 0;
    ...
    if (reg_np->dev->node)
    {
        rt_dtb_data(reg_np->dev->node) = reg_np;
    }
...
rt_err_t rt_regulator_unregister(struct rt_regulator_node *reg_np)
{
    ...
    if (!rt_list_isempty(&reg_np->children_nodes) || reg_np->open_count > 1)
    {
        err = -RT_EBUSY;
...
rt_err_t rt_regulator_notifier_unregister(struct rt_regulator *reg,
        struct rt_regulator_notifier *notifier)
{
    struct rt_regulator_node *reg_np = reg->reg_np;
...
rt_bool_t rt_regulator_is_enabled(struct rt_regulator *reg)
{
    ...
    return reg->reg_np->enabled_count > 0;
}
...
struct rt_regulator *rt_regulator_get(struct dtb_node *np, const char *id)
{
    ...
    if (np)
    {
        ...
        if (dtb_node_read_u32(np, supply_name, &supply_phandle))
        {
            goto _end;
        }

        if (!(np = dtb_node_find_node_by_phandle(supply_phandle)))
        {
